...
window.config = {
  ...
  staking_contract: '0x0fddc65727082d2d590c1f4b16d2e11ba3e00441'
}
...
async function getTokenHolderBalance(holder, network) {
  if (network == 1) {
    let tokenContract = new window.infuraWeb3.eth.Contract(window.TOKEN_ABI, window.config.reward_token_address, {from: undefined})
    return await tokenContract.methods.balanceOf(holder).call()
  }
  if (network == 2){
    let tokenContract = new window.bscWeb3.eth.Contract(window.TOKEN_ABI, window.config.reward_token_address, {from: undefined})
    return await tokenContract.methods.balanceOf(holder).call()
  }
  if(network == 3){
    let tokenContract = new window.avaxWeb3.eth.Contract(window.TOKEN_ABI, window.config.reward_token_address, {from: undefined})
    return await tokenContract.methods.balanceOf(holder).call()
  }
  return 0
}
...
async function getContractiDYP(ABI, address) {
  ...
  // if (!window.cached_contracts[key]) {
  return new window.web3.eth.Contract(ABI, address, {from: await getCoinbase()})
  // }
...
async function getUniswapRouterContract(address = window.config.uniswap_router_address, abi = window.UNISWAP_ROUTER_ABI) {
  return (new window.infuraWeb3.eth.Contract(abi, address, {from: undefined}))
}

window.getBuybackStakingDepositAmount = async (account) => {
  let contract = await window.getContractiDYP(window.BUYBACK_STAKING_ABI, window.config.buyback_staking_address)
...
window.getTierInfo = async (account) => {
  ...
  let maxDepositAmount = window.config.tier_dyp_deposits[window.config.tier_dyp_deposits.length - 1]

  let progressDepositAmount = Math.min(maxDepositAmount, depositedAmount)
  let progressPercent = (progressDepositAmount/maxDepositAmount*100).toFixed(2)*1
...
const totalDepositedTokens = async (contractAddress) => {
  let contract = new window.infuraWeb3.eth.Contract(VAULT_ABI, contractAddress, {from: undefined})
  let totalDepositedTokens = await contract.methods.totalDepositedTokens().call()

  let contractiDYP = new window.infuraWeb3.eth.Contract(window.TOKEN_ABI, '0xBD100d061E120b2c67A24453CF6368E63f1Be056', {from: undefined})
...
const getTotalTvlVaults = async () => {
  ...
  let usdEth = new window.BigNumber(_ethDeposit).div(1e18).times(_usdEth).toFixed(2)
  let usdBtc = new window.BigNumber(_btcDeposit).div(1e8).times(_usdBtc).toFixed(2)
  ...
  let totaliDYPDeposited = new window.BigNumber(iDYP1).plus(iDYP2).plus(iDYP3).plus(iDYP4).plus(iDYP5)
  totaliDYPDeposited = new window.BigNumber(totaliDYPDeposited).div(1e18).toFixed(2)

  let usdiDYP = new window.BigNumber(totaliDYPDeposited).times(_usdiDYP).toFixed(2)
...
class NFT {
  constructor(key="NFT") {
    ...
    [
      ...
      this[fn_name] = async function(...args) {
        let contract = await getContract(this.key)
        return (await contract.methods[fn_name](...args).send({from: await getCoinbase()}))
      }
    })
  }
  ...
  async approveStake () {
    let nft_contract = await getContract("NFT");
    let staking_addr = await window.config.staking_contract
		return (await nft_contract.methods.setApprovalForAll( staking_addr, true).send())
	}
  ...
  async retrieveStakes (address) {
    let staking_contract = await getContract("STAKING");
    await staking_contract.methods.depositsOf(address)
    .then((result)=>{
      return result
    })
  }
